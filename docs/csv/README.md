# Dossiê CSV — ProcVault QMS (rascunho DevSistem)

**Estado:** rascunho técnico. **Não** é registo oficial da Qualidade (sem vistos, sem evidências de execução).  
**Data:** 15/09/2026  
**Produto:** QualiProc / ProcVault QMS — versão npm `0.1.0`  
**Classificação:** software GAMP **Classe 3** (customizado) sobre infra GAMP 1 (Vercel + Supabase).  
**Afirmação comercial “sistema validado BPx”:** **NO-GO** até execução aprovada dos protocolos e RFV com vistos GQ.

O pack DevSistem **não inclui** o formulário controlado FOR-SGQ-010A no SGQ da empresa. Enquanto a Qualidade não o adotar, aplica-se o **ciclo GAMP Classe 3** abaixo. Se a Qualidade adotar FOR-SGQ-010A, **esse protocolo prevalece** — não inventar QI/QO GAMP genérico em duplicado.

| Documento | Ficheiro |
|-----------|----------|
| Inventário de sistema | [00-INVENTARIO.md](./00-INVENTARIO.md) |
| ERU | [01-ERU.md](./01-ERU.md) |
| Análise de riscos | [02-ANALISE-RISCOS.md](./02-ANALISE-RISCOS.md) |
| Plano de Validação | [03-PLANO-VALIDACAO.md](./03-PLANO-VALIDACAO.md) |
| Matriz REQ–RISK–DES–TEST | [04-MATRIZ-RASTREABILIDADE.md](./04-MATRIZ-RASTREABILIDADE.md) |
| FOR-SGQ-010A (rascunho) | [FOR-SGQ-010A.md](./FOR-SGQ-010A.md) |
| CC-010 controlos eletrónicos | [CC-010-CONTROLOS-ELETRONICOS.md](./CC-010-CONTROLOS-ELETRONICOS.md) |
| CC-011 endurecimento live CTLI | [CC-011-VALIDACAO-HARDENING.md](./CC-011-VALIDACAO-HARDENING.md) |
| Relatório final (pré-execução) | [05-RFV.md](./05-RFV.md) |
| QO fórmulas | [QO-FORMULAS.md](./QO-FORMULAS.md) |
| QO proposta → coleta → cert | [QO-PROPOSTA-COLETA-CERT.md](./QO-PROPOSTA-COLETA-CERT.md) |
| LGPD / DPA | [LGPD-DPA.md](./LGPD-DPA.md) |
| Backup QI/QO | [../11-BACKUP-DR-QIQO.md](../11-BACKUP-DR-QIQO.md) |
| Reverificação live (15/09/2026) | [06-REVERIFICACAO-LIVE.md](./06-REVERIFICACAO-LIVE.md) |

**Exclusões documentais (não há workflow):** reclamações PR-7.9, NC PR-7.10, CAPA PR-8.7.  
**Fora de escopo BPx:** produção farmacêutica, pesagem de formulação, comando de equipamento.
