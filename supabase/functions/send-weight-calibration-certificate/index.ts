  ...
  try {
    ...
    if (action === "send_zip") {
      ...
      try {
        ...
        await adminClient.from("weight_certificate_email_deliveries").insert(
          certs.map((c) => ({
            certificate_id: c.id,
            tenant_id: tenantId,
      ...
      await adminClient.from("weight_calibration_certificates").update({
        status: "enviado",
    ...
    if (action === "notify") {
      ...
      const resendData = await sendViaResend({
        ...
        html: `
          <p>Olá,</p>
          <p>O certificado <strong>${certLabel}</strong> (${cert.client_name || "—"}, identificação ${cert.weight_tag || "—"}) aguarda sua aprovação.</p>
    ...
    try {
      resendData = await sendViaResend({
        ...
        html: `
          ...
          <p>Identificação do peso: <strong>${cert.weight_tag || "—"}</strong>.</p>
          <p>Atenciosamente,<br/>${fromName}</p>
        `,
