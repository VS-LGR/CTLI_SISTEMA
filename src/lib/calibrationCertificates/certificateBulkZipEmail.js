...
import { blobToBase64, isValidEmail, resolveClientEmail } from "@/lib/certificateEmail/certificateEmailApi";
import {
  MAX_ZIP_EMAIL_BYTES,
 ...
 */
export async function sendCertificatesZipByEmail({
  ids = [],
  loadCertificate,
  tenant = null,
  tenantName = "",
  ...
  const { blob, ok, fail, errors } = await buildCertificatesZipBlob({
    ids: list,
