        ...
        <table className="w-full text-sm min-w-[720px]">
          ...
          <tbody className="divide-y divide-border bg-card">
            ...
            {!loading && rows.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">Nenhuma alteração registada.</td></tr>
            )}
            {rows.map((row) => {
              ...
              return (
                <tr key={row.id} className="align-top">
                  ...
                  <td className="px-3 py-2 text-xs">
                    <div className="font-medium text-foreground">{row.user_full_name || "—"}</div>
                    <div className="text-muted-foreground">{row.user_email || "—"}</div>
                    <div className="text-muted-foreground">{roleLabel(row.user_role)}</div>
                  </td>
                  <td className="px-3 py-2 text-xs">{row.user_function || "—"}</td>
                  <td className="px-3 py-2 text-xs">
                    <p className="text-foreground/90 mb-1">{row.summary || "—"}</p>
                    {entries.length > 0 && (
                      <ul className="space-y-0.5 text-muted-foreground">
                        {entries.slice(0, 8).map((e) => (
                          <li key={e.field}>
                            ...
                            {" → "}
                            <span className="text-foreground">{String(e.to)}</span>
                          </li>
                        ))}
                        {entries.length > 8 && (
                          <li className="text-muted-foreground">+{entries.length - 8} campos…</li>
                        )}
                      </ul>
                    )}
                  </td>
