...
import { DEFAULT_PROPOSAL_BOILERPLATE } from "@/lib/commercialProposals/commercialProposalDocMeta";
...
export default function CommercialProposalTenantConfig({ tenantId, tenant, onSaved }) {
  ...
  const save = async () => {
    ...
    const { error } = await supabase
      .from("tenants")
      .update({ commercial_proposal_boilerplate: boilerplate })
      .eq("id", tenantId);
    setSaving(false);
    if (error) toast.error(error.message);
    else {
      toast.success("Textos institucionais RE-7.1A guardados");
      onSaved?.();
    }
  };
  ...
  return (
    <Card className="border-slate-200 max-w-3xl">
      <CardContent className="p-4 space-y-6">
        ...
        <Button onClick={save} disabled={saving}>{saving ? "Guardando…" : "Guardar textos"}</Button>
      </CardContent>
    </Card>
  );
}
