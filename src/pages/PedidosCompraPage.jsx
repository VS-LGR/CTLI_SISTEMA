...
export default function PedidosCompraPage() {
  ...
  if (!canAccessPurchaseOrders(user?.role, user)) {
    return <div className="text-muted-foreground">Sem permissão para pedidos de compra.</div>;
  }
  if (!isSupabaseAuthMode) {
    return <div className="text-muted-foreground">Pedidos de compra requerem modo Supabase.</div>;
  }
  if (!currentTenantId) {
  ...
  return (
    <div className="space-y-6 min-w-0" data-testid="pedidos-compra-page">
      <div>
        ...
        <h1 className="font-display text-2xl sm:text-3xl font-bold text-foreground">Pedidos de Compra</h1>
        <p className="text-sm text-muted-foreground mt-1">Ambiente: {currentTenant?.name}</p>
      </div>
